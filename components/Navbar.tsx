...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 gap-6">
            ...
            <Link href="/" className="flex items-center gap-2.5 shrink-0 group">
              <span className="w-9 h-9 rounded bg-[#C0392B] flex items-center justify-center text-white text-lg font-bold leading-none transition-transform group-hover:scale-105">
                ✚
              </span>
              <span className="flex flex-col leading-none">
                <span className="font-serif font-extrabold text-xl text-[#1C1C1E] tracking-tight leading-none">MedAir</span>
            ...
            >
              <span className={`block w-5 h-0.5 bg-[#1C1C1E] rounded-full transition-all duration-300 origin-center ${open ? 'rotate-45 translate-y-2' : ''}`} />
              <span className={`block w-5 h-0.5 bg-[#1C1C1E] rounded-full transition-all duration-300 ${open ? 'opacity-0 scale-x-0' : ''}`} />
              <span className={`block w-5 h-0.5 bg-[#1C1C1E] rounded-full transition-all duration-300 origin-center ${open ? '-rotate-45 -translate-y-2' : ''}`} />
            </button>
          </div>
        </div>
      ...
      {/* Mobile Overlay */}
      <div className={`fixed inset-0 z-30 lg:hidden transition-all duration-300 ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-72 bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${open ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
            <span className="font-serif font-bold text-lg text-[#1C1C1E]">MedAir <span className="text-[#C0392B]">Global</span></span>
            <button onClick={() => setOpen(false)} aria-label="Close menu" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-stone-100 transition-colors">
